...
function TableHeader({ className, ...props }: React.ComponentProps<"thead">) {
  return <thead className={cn("border-b border-slate-200 bg-slate-50 [&_tr]:border-b-0", className)} {...props} />;
}
...
function TableHead({ className, ...props }: React.ComponentProps<"th">) {
  return (
    <th
      className={cn(
        "h-11 px-6 text-left align-middle text-sm font-semibold text-slate-900 whitespace-nowrap",
        className
      )}
      {...props}
    />
...
function TableCell({ className, ...props }: React.ComponentProps<"td">) {
  return <td className={cn("px-6 py-3 align-middle text-sm text-slate-700", className)} {...props} />;
}
